...
#from dash import 
from dash import Input, Output, Dash, callback, html as html, dcc as dcc
...
external_stylesheets = [r'styles/bWLwgP.css']
...
def tab3_weekday_sales(store, start_date, end_date):

    truncated = df.merged[(df.merged['tran_date'] >= start_date) &
                          (df.merged['tran_date'] <= end_date)]

    data = truncated[truncated['Store_type'] == store]

    grouped = data.groupby('weekday')['total_amt'].sum().reindex([
    ...
    fig.update_layout(title=f"Sprzedaż wg dni tygodnia – {store}",
...
@app.callback(
    Output('customer-basket', 'figure'),
    [Input('store-dropdown', 'value')]
)
def tab3_customer_basket(store):
    data = df.merged[(df.merged['Store_type'] == store) &
                     (df.merged['total_amt'] > 0)]

    grouped = data.groupby('cust_id')['total_amt'].mean()

    fig = go.Figure([go.Histogram(x=grouped.values)])
    fig.update_layout(title="Średnia wartość koszyka na klienta",
